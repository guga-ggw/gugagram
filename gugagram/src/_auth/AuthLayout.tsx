            ...
            <>
             ...
             <motion.div initial={{y : -100, x : 100, opacity : 0}} animate={{opacity : 1, y:0, x : 0}} transition={{duration : .9, delay : 0}} className={`absolute ${Number(height) < 700 ? "w-40 h-40 pt-8" : Number(height) < 800 ? "w-48 h-48"  : "w-56 h-56" } rounded-tl-[20%] rounded-bl-full flex justify-center items-center pb-10 pl-10 bg-[rgb(223,204,198)] right-0 top-0 lg:hidden`}>
              <ul className='flex flex-col gap-4'>
                <motion.li  initial={{opacity : 0, x : -15}} animate={{opacity : 1, x : 0}} transition={{duration : 1, delay : 0.8}} className={`${Number(height) < 700 ? "text-sm" : "text-xl"}`}>Sign up</motion.li>
                <motion.li initial={{opacity : 0, x : -15}} animate={{opacity : 1, x : 0}} transition={{duration : 1, delay : 1}}className={`${Number(height) < 700 ? "text-sm" : "text-xl"}`}>Sign up</motion.li>
              </ul>
             </motion.div>
            </>
            : 
